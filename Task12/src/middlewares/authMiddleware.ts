...
class AuthMiddleware {
    ...
    public async checkActiveToken(req:IRequestExtend, res:Response, next:NextFunction) {
        try {
            ...
            const { userEmail } = tokenService.verifyToken(activeToken, 'active');

            const tokenFromDb = await tokenRepository.findActiveTokenByParams(
                { activeToken },
            );
